...
import { QrCode, UploadCloud, Info, ArrowLeft, CheckCircle2, Copy, History } from 'lucide-react';
...
const Deposit = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post('http://localhost:5000/api/transactions/deposit', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            setSuccess(true);
        ...
        <div className="flex flex-col" style={{ gap: 28, paddingBottom: 100 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ padding: 22 }}>
                <div className="flex justify-between items-center" style={{ marginBottom: 18 }}>
                    ...
                    <div className="flex items-center gap-1" style={{ padding: '4px 10px', borderRadius: 20, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)' }}>
